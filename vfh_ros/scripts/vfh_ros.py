...
from metrics import PathSmoothnessMetric
...
class NodeVFH():
    ...
    def __init__(self):
        ...
        VFH params
        """
        self.threshhold = rospy.get_param('~threshhold', 0.5)
        self.sector_num = rospy.get_param('~sector_num', 100)
        self.wide_sectors = rospy.get_param('~wide_sectors', 5)
        self.safe_radius = rospy.get_param('~safe_radius', 0.4)
    ...
    def cmd(self, angle):
        ...
        turn_speed = 0.0
        linear_speed = 0.0
        if self._destination_distance > 0.1:
            ...
            smoothness, total_distance = self.metric.report()
            time_diff = rospy.Time.now() - self._time
            # calculate time to seconds
            time_diff = time_diff.secs + time_diff.nsecs / 1e9
            rospy.loginfo("Frame ID: %s, Path Smoothness: %s, Total distance: %s, Time: %s", self.frame_id, smoothness, total_distance, time_diff)
    ...
    def odom_callback(self, msg):
        ...
        self.metric.update_orientation(msg.pose.pose.position.x, msg.pose.pose.position.y, yaw)
