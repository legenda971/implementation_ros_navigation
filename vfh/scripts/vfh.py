...
class Valley:
    ...
    def __init__(self, start, end, total_cost, size):
        """
        ...
        :param total_cost: Float representing the total cost (sum of values) of the valley.
    ...
    # def in_valley(self, sector, sector_num, safe_sector = 4):
    #     """
    #     Determines whether a given sector is within the valley.

    #     :param sector: Integer representing the sector index to check.
    #     :param sector_num: Integer representing the number of sectors in the histogram.
    #     :param safe_radius: Integer representing the safe radius of the robot, in vfh this variable represent r_s.
        
    #     :return: Boolean value indicating whether the sector is within the valley.
    #     """
...
class VFH():
    ...
    def calculate(self, occupancyGrid, destination_angle):
        ...
        # print("-----------------------------------")
        # for valley in valleys:
        #     print("Valley: ", valley.start, valley.end, valley.total_cost, valley.size)
        # print("Destination sector: ", des_sector, "best_sector: ", best_sector, self.histogram[best_sector])
        # for sector in candidate_sectors:
        #     print("Candidate sector: ", sector, "distance", self.circular_distance(sector, des_sector) + self.circular_distance(sector, self.sector_num))
        ...
        for i in range(n * 2):
            ...
            if histogram[idx] <= threshold:
                if not in_valley:
                    start_idx = idx
                    in_valley = True
                total_cost += histogram[idx]
                count += 1
            else:
                if in_valley:
                    end_idx = (i - 1) % n
                    valleys.append(Valley(start_idx, end_idx, total_cost, count))
                    in_valley = False
                    total_cost = 0
                    count = 0
